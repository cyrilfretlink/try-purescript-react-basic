...
const matchErrorsSeparator = /Error \d+ of \d+:/;
...
module.exports = function purescriptLoader(source, map) {
  ...
  // BRK
  // if (!/.psc-package/.test(this.resourcePath)) debugger;
  ...
  var loaderSrc = loaderOptions.src || [
    path.join('src', '**', '*.purs')
  ];
  ...
  if (!CACHE_VAR.installed) {
    ...
    this._compiler.plugin('invalid', function () {
      ...
      CACHE_VAR = {
        ...
        warnings: [],
        errors: [],
        compiledOnce: CACHE_VAR.compiledOnce,
        compilationStarted: false,
        compilationFinished: false,
  ...
  var psModule = {
    ...
    emitError: pscMessage => {
      if (pscMessage.length) {
        ...
        const errors = splitPscErrors(pscMessage);
        for (const error of errors) {
          const [, relname] = matchLocation.exec(error) || [];
          if (!relname) continue;

          const filename = path.join(this.rootContext, relname);
          this.addDependency(filename);

          const [, fromModuleName] = matchModuleName.exec(error) || [];
          if (!fromModuleName) continue;
          ...
          if (typeof this.extractDependencies === 'function') {
            for (const dep of this.extractDependencies(error)) {
        ...
        if (error instanceof ide.UnknownModuleError) {
          ...
          if (!CACHE_VAR.compilationStarted) {
            ...
            return compile(psModule).then(function () {
              ...
              // globby(loaderSrc, { cwd: _this.rootContext }).then(function (srcs) {
              //   srcs.forEach(src => _this.addDependency(src));
