...
module.exports = function purescriptLoader(source, map) {
  ...
  const srcOption = (pscPackage => {
    ...
    if (CACHE_VAR.srcOption.length > 0) {
      ...
      const loaderSrc = loaderOptions.src || [srcPath];
      ...
      const result = loaderOptions.src || [bowerPath, srcPath];
  ...
  const psModule = {
    ...
    emitError: pscMessage => {
      if (pscMessage.length) {
        ...
        const errors = utils.splitPscErrors(pscMessage);
        for (const error of errors) {
          const [, relative] = utils.matchLocation.exec(error) || [];
          if (!relative) continue;

          const filename = path.join(this.rootContext, relative);
          this.addDependency(filename);

          const [, baseModuleName] = utils.matchModuleName.exec(error) || [];
          if (!baseModuleName) continue;

          for (const re of [utils.matchMissingModuleName, utils.matchMissingImportFromModuleName]) {
            const [, targetModuleName] = re.exec(error) || [];
            if (targetModuleName) {
              const resolved = utils.resolvePursModule({
                baseModulePath: filename,
                baseModuleName,
                targetModuleName
              });
          ...
          if (typeof this.extractDependenciesOnError === 'function') {
            for (const dep of this.extractDependenciesOnError(error)) {
          ...
          if (!CACHE_VAR.compilationStarted) {
            ...
            return compile(psModule)
              ...
              .catch(error => {
                ...
                CACHE_VAR.deferred[0].reject(Object.assign(error, {
                  psMainModule: psModule.name
                }));
